...
#include <string>
#endif  // UNIT_TEST
// 统一使用 volatile, 不使用 std::atomic, 避免 C++20 下类型歧义
// 重命名以避免和 std::atomic_uint16_t 等冲突
typedef volatile bool ir_atomic_bool;
typedef volatile uint16_t ir_atomic_uint16_t;
typedef volatile uint32_t ir_atomic_uint32_t;
typedef volatile const uint16_t ir_atomic_const_uint16_t;
