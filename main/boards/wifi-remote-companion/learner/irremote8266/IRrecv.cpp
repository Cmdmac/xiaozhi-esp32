...
bool IRrecv::decode(decode_results *results, irparams_t *save,
  ...
  for (uint16_t offset = kStartOffset;
       ...
       offset += 2) {
// ============================================================
// 精简版 IRrecv.cpp 未包含各协议解码实现 (decodeNEC/decodeSanyo 等),
// 若编译这些调用会产生未定义引用 (此前一直由完整版 IRremoteESP8266 提供)。
// 因此协议解码区整体关闭 (#if 0), 学习统一走 decodeHash:
// 所有码按 UNKNOWN + 完整 raw 码保存, 回放直接用 rawData, 协议无关。
// 如需启用某协议, 把对应 #if DECODE_XXX 块恢复, 并补上该协议实现即可。
// ============================================================
#if 0
...
#endif  // DECODE_EUROM
#endif  // #if 0: 协议解码区整体关闭 (不含 for 循环闭合大括号), 学习统一走 decodeHash
...
match_result_t IRrecv::matchData(
    ir_atomic_uint16_t *data_ptr, const uint16_t nbits, const uint16_t onemark,
...
uint16_t IRrecv::matchBytes(ir_atomic_uint16_t *data_ptr, uint8_t *result_ptr,
...
///   true is Most Significant Bit First Order, false is Least Significant First
/// @return If successful, how many buffer entries were used. Otherwise 0.
uint16_t IRrecv::_matchGeneric(ir_atomic_uint16_t *data_ptr,
...
///   true is Most Significant Bit First Order, false is Least Significant First
/// @return If successful, how many buffer entries were used. Otherwise 0.
uint16_t IRrecv::matchGeneric(ir_atomic_uint16_t *data_ptr,
                              uint64_t *result_ptr,
                              const uint16_t remaining,
                              const uint16_t nbits,
                              const uint16_t hdrmark,
...
uint16_t IRrecv::matchGenericConstBitTime(ir_atomic_uint16_t *data_ptr,
                                          uint64_t *result_ptr,
                                          const uint16_t remaining,
                                          const uint16_t nbits,
                                          const uint16_t hdrmark,
...
uint16_t IRrecv::matchManchester(ir_atomic_const_uint16_t *data_ptr,
                                 uint64_t *result_ptr,
                                 const uint16_t remaining,
                                 const uint16_t nbits,
                                 const uint16_t hdrmark,
...
uint16_t IRrecv::matchManchesterData(ir_atomic_const_uint16_t *data_ptr,
                                     uint64_t *result_ptr,
                                     const uint16_t remaining,
                                     const uint16_t nbits,
                                     const uint16_t half_period,
